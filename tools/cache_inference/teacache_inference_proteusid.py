...
def teacache_forward(
    ...
    #   - It is okay to `channels` use for ProteusID (number of input channels is equal to output channels)
...
def main(args):
    ...
    # ProteusID works well with long and well-described prompts. Make sure the face in the image is clearly visible (e.g., preferably half-body or full-body).
    prompt = args.prompt
    image = args.image
    
    if not os.path.exists(ckpts_path):
        print("Base Model not found, downloading from Hugging Face...")
        snapshot_download(repo_id="BestWishYsh/ProteusID-preview", local_dir=ckpts_path)
    else:
    ...
    pipe = ProteusIDPipeline.from_pretrained(ckpts_path, torch_dtype=torch.bfloat16)
    pipe.to("cuda")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run ProteusID with given parameters")
    ...
    parser.add_argument('--ckpts_path', type=str, default="BestWishYsh/ProteusID-preview", help='Path to checkpoint')
    # higher speedup will cause to worse quality -- 0.1 for 1.6x speedup -- 0.15 for 2.1x speedup -- 0.2 for 2.5x speedup
    ...
    parser.add_argument('--image', type=str, default="https://github.com/PKU-YuanGroup/ProteusID/blob/main/asserts/example_images/2.png?raw=true", help='URL or path to input image')
