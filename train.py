...
from models.proteusid_utils import (
...
from models.dataloader import ProteusID_Dataset, RandomSampler, SequentialSampler
from models.pipeline_cogvideox import CogVideoXPipeline
from models.pipeline_proteusid import ProteusIDPipeline, draw_kps 
...
def main(args):
    ...
    transformer_additional_kwargs={
        ...
        'FT_txt_dim': args.FT_txt_dim,
        'FT_clip_dim': args.FT_clip_dim,
        'FT_vit_dim': args.FT_vit_dim,
        'FT_num_querie': args.FT_num_querie,
        'FT_depth': args.FT_depth,
        'FT_dim_head': args.FT_dim_head,
        'FT_num_heads': args.FT_num_heads,
        'FT_output_dim': args.FT_output_dim,
        'FT_num_scale': args.FT_num_scale,
        'FT_ff_mult': args.FT_ff_mult,
        'FFE_num_queries': args.FFE_num_queries,
        'FFE_query_dim': args.FFE_query_dim,
        'FFE_input_img_dim': args.FFE_input_img_dim,
        'FFE_input_fuse_dim': args.FFE_input_fuse_dim,
        'FFE_output_dim': args.FFE_output_dim,
        'FFE_layers': args.FFE_layers,
        'FFE_heads': args.FFE_heads,
        'FFE_clip_dim': args.FFE_clip_dim,
        'FFE_vit_dim': args.FFE_vit_dim,
        'FFE_time_channel': args.FFE_time_channel,
        'FFE_time_embed_dim': args.FFE_time_embed_dim,
        'FFE_timestep_activation_fn': args.FFE_timestep_activation_fn
    }
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        # Load RAFT model from local weights file
        raft_weight_path = "raft_weight/raft_large_C_T_SKHT_V2-ff5fadd5.pth"
        raft_estimator = raft_large(weights=None, progress=False)
        raft_state_dict = torch.load(raft_weight_path, map_location="cpu")
        raft_estimator.load_state_dict(raft_state_dict)
        logger.info(f"Loaded RAFT weights from {raft_weight_path}")
        ...
        face_helper_1 = None
        face_helper_2 = None
        # Always initialize face models (is_train_face, is_fuse_token, is_fuse_token_v2 are always True)
    ...
    raft_estimator.requires_grad_(False)
    ...
    # Always freeze face models (is_train_face is always True)
    ...
    raft_estimator.to(accelerator.device, dtype=torch.float32)
    
    
    # Always move face models to device (is_train_face is always True)
    ...
    if args.use_ema:
        ema_transformer3d = ProteusIDTransformer3DModel.from_pretrained_cus(
            args.pretrained_model_name_or_path if args.pretrained_weight is None else args.pretrained_weight,
            subfolder="transformer",
            config_path=args.config_path,
            transformer_additional_kwargs=transformer_additional_kwargs,
        ...
        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=ProteusIDTransformer3DModel, model_config=ema_transformer3d.config)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            if args.is_train_lora:
                ProteusIDPipeline.save_lora_weights(
                    output_dir,
    ...
    def load_model_hook(models, input_dir):
        if args.use_ema:
            ...
            _, ema_kwargs = ProteusIDTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
            load_model = ProteusIDTransformer3DModel.from_pretrained_cus(
                input_dir,
                subfolder="transformer_ema",
                transformer_additional_kwargs=transformer_additional_kwargs,
            )

            load_model = EMAModel(load_model.parameters(), model_cls=ProteusIDTransformer3DModel, model_config=load_model.config)
        ...
        for i in range(len(models)):
            ...
            if args.is_train_lora:
                lora_state_dict = ProteusIDPipeline.lora_state_dict(input_dir)
    ...
    # Always unfreeze these modules (is_train_face, is_fuse_token, is_fuse_token_v2 are always True)
    unfreeze_modules = ["fuse_facital_attention", "perceiver_cross_attention", "fuse_token_former", "fuse_token_adapter"]
    ...
    train_dataset = ProteusID_Dataset(
    ...
    if accelerator.is_main_process:
        tracker_name = args.tracker_name or "proteusid-ipt2v"
    ...
    def compute_prompt_embeddings_for_batch(prompts):
        return zip(*[compute_prompt_embeddings(
                    tokenizer,
                    text_encoder,
                    [prompt],
    ...
    def compute_identity_prompt_embeddings_for_batch(prompts):
        return [
                compute_prompt_embeddings(
                tokenizer,
                text_encoder,
    ...
    def compute_raft(video_tensor, human_mask, temporal_batch_size=10):
        """
        Args:
            video_tensor: [B, T, C, H, W]
            human_mask: [B, T, H, W]
            temporal_batch_size: batch size for temporal dimension
        """
        B, T, C, H, W = video_tensor.shape
        device = video_tensor.device
        
        # Pre-allocate result tensor
        dis_batch = torch.zeros(B, T-1, H, W, device=device, dtype=video_tensor.dtype)
        
        # Process all consecutive frame pairs
        for i in range(0, T-1, temporal_batch_size):
            end_idx = min(i + temporal_batch_size, T-1)
            curr_batch_size = end_idx - i
            
            # Prepare batch data
            img1_batch = video_tensor[:, i:end_idx].reshape(-1, C, H, W)
            img2_batch = video_tensor[:, i+1:end_idx+1].reshape(-1, C, H, W)
            
            img1_batch, img2_batch = raft_transform(img1_batch, img2_batch)
            
            # Predict optical flow
            predicted_flow = raft_estimator(img1_batch, img2_batch)
            predicted_flow = predicted_flow[-1]
            
            # Calculate displacement magnitude
            magnitude = torch.sqrt(predicted_flow[:, 0]**2 + predicted_flow[:, 1]**2)
            magnitude = magnitude.reshape(B, curr_batch_size, H, W)
            
            # Directly write to pre-allocated tensor
            dis_batch[:, i:end_idx] = magnitude
        
        # Copy the last frame
        dis_last_step = dis_batch[:, -1:, :, :]
        dis_batch_extended = torch.cat([dis_batch, dis_last_step], dim=1)
        
        # Apply mask and sigmoid
        dis_batch_extended = dis_batch_extended * human_mask
        nonzero_mask = (dis_batch_extended != 0)
        
        normalized_masked_dis_batch = dis_batch_extended.clone()
        normalized_masked_dis_batch[nonzero_mask] = torch.sigmoid(
            dis_batch_extended[nonzero_mask]
        )
        
        return normalized_masked_dis_batch
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                if args.low_vram:
                    ...
                    raft_estimator.to(accelerator.device)
                    # Always move face models to device (is_train_face is always True)
                ...
                with torch.no_grad():
                    ...
                    batch_identity_prompt = batch["identity_prompt"]
                    batch_raft_video = batch["raft_video"]

                    # Always process face data (is_train_face is always True)
                    if args.enable_mask_loss and random.random() < 0.5:
                        enable_mask_loss_flag = True
                    else:
                        enable_mask_loss_flag = False

                    # Always process face data (is_train_face is always True)
                    expand_face_imgs = batch['expand_face_imgs'] if batch['expand_face_imgs'] is not None else None
                    original_face_imgs = batch['original_face_imgs'] if batch['original_face_imgs'] is not None else None
                    if args.is_reserve_face:
                        reserve_face_imgs = batch["reserve_face_imgs"]
                    
                    
                    human_dense_masks = batch['human_masks_tensor'].to(memory_format=torch.contiguous_format).to(dtype=weight_dtype) if batch['human_masks_tensor'] is not None else None
                    
                    if args.enable_mask_loss and enable_mask_loss_flag:
                        dense_masks = batch['dense_masks_tensor'].to(memory_format=torch.contiguous_format).to(dtype=weight_dtype) if batch['dense_masks_tensor'] is not None else None
                    ...
                    if face_clip_model is not None and face_main_model is not None:
                        ...
                        expand_face_imgs = torch.stack(expand_valid_face_imgs)
                        original_face_imgs = torch.stack(original_valid_face_imgs)
                        ...
                        valid_indices = []
                        face_kps_list = []
                        for idx, id_image in enumerate(expand_face_imgs):
                            ...
                            try:
                                ...
                                # id_image is the face image
                                # align_crop_face_image is the image with only face
                                # face_kps are face keypoint coordinates
                        ...
                        if len(valid_id_conds) != 0:
                            ...
                            if len(valid_id_conds) != B:
                                ...
                                batch_raft_video = torch.stack([batch_raft_video[i] for i in valid_indices])
                                batch_instance_prompt = [batch_instance_prompt[i] for i in valid_indices]
                                batch_identity_prompt = [batch_identity_prompt[i] for i in valid_indices]
                                
                                if args.enable_mask_loss and enable_mask_loss_flag:
                                    dense_masks = torch.stack([dense_masks[i] for i in valid_indices])
                                    
                                human_dense_masks = torch.stack([human_dense_masks[i] for i in valid_indices])
                                print(f"Adjusted batch size from {B} to {len(valid_id_conds)} due to mismatched valid_id_conds length.")

                    # Always process video latents (is_train_face is always True, so check valid_indices)
                    if len(valid_indices) != 0:
                        video_latents_list = []
                        image_latents_list = []
                        for idx, (video, face_img) in enumerate(zip(batch_instance_video, align_crop_face_imgs)):
                            # Always use face processing path (is_train_face is always True)
                            ...
                            if args.train_type == 'i2v':
                                image = video_processor.preprocess(pil_img, height=args.height, width=args.width).to(memory_format=torch.contiguous_format).to(accelerator.device, dtype=weight_dtype)
                                image_latent_dist = process_image(image.unsqueeze(2), vae)
                            if args.is_kps:
                                valid_face_kps = draw_kps(pil_img, face_kps_list[idx])
                                valid_face_kps = video_processor.preprocess(valid_face_kps, height=args.height, width=args.width).to(memory_format=torch.contiguous_format).to(accelerator.device, dtype=weight_dtype)
                                face_kps_latent_dist = process_image(valid_face_kps.unsqueeze(2), vae)
                            ...
                            if args.train_type == 'i2v':
                                ...
                                if args.is_kps:
                                    face_kps_latents = face_kps_latent_dist.sample() * VAE_SCALING_FACTOR
                                    face_kps_latents = face_kps_latents.permute(0, 2, 1, 3, 4)
                        ...
                        prompt_embeds, prompt_masks = compute_prompt_embeddings_for_batch(batch_instance_prompt)
                        prompt_embeds = torch.cat(prompt_embeds)
                        prompt_masks = torch.cat(prompt_masks)
                        
                        identity_prompt_embeds = torch.cat(compute_identity_prompt_embeddings_for_batch(batch_identity_prompt))
                ...
                if args.low_vram:
                    vae.to('cpu')
                    text_encoder.to('cpu')
                    raft_estimator.to('cpu')
                    # Always move face models to CPU (is_train_face is always True)
                    face_clip_model.to('cpu')
                    face_helper_1.face_det.to('cpu')
                    face_helper_1.face_parse.to('cpu')
                    free_memory()

                # Always process training (is_train_face is always True, so check valid_indices)
                if len(valid_indices) != 0:
                    ...
                    raft_weight = Raft_Large_Weights.DEFAULT
                    raft_transform = raft_weight.transforms()

                    opflow = compute_raft(batch_raft_video, human_dense_masks, temporal_batch_size=5)

                    # enable mask loss (is_train_face is always True)
                    if args.enable_mask_loss and enable_mask_loss_flag:
                        dense_masks = dense_masks.unsqueeze(1)
                        temp_video_latents = torch.zeros((batch_size, num_channels, num_frames, height, width))
                        dense_masks = resize_mask(dense_masks, temp_video_latents, process_first_frame_only=False)
                        
                        if patch_size_t is not None:
                            ncopy = dense_masks.shape[2] % patch_size_t
                            # Copy the first frame ncopy times to match patch_size_t
                            first_frame = dense_masks[:, :, :1, :, :]
                            dense_masks = torch.cat([first_frame.repeat(1, 1, ncopy, 1, 1), dense_masks], dim=2)
                            assert dense_masks.shape[2] % patch_size_t == 0
                        dense_masks = dense_masks.repeat(1, temp_video_latents.shape[1], 1, 1, 1).permute(0, 2, 1, 3, 4).float()
                        dense_masks = dense_masks.reshape(video_latents.shape[0], -1)
                        dense_masks = dense_masks.to(accelerator.device)
                    
                    opflow = opflow.unsqueeze(1) 
                    temp_video_latents = torch.zeros((batch_size, num_channels, num_frames, height, width))                        
                    opflow = resize_mask(opflow, temp_video_latents, process_first_frame_only=False)
                    if patch_size_t is not None:
                        ncopy = opflow.shape[2] % patch_size_t
                        # Copy the first frame ncopy times to match patch_size_t
                        first_frame = opflow[:, :, :1, :, :]
                        opflow = torch.cat([first_frame.repeat(1, 1, ncopy, 1, 1), opflow], dim=2)
                        assert opflow.shape[2] % patch_size_t == 0
                    opflow = opflow.repeat(1, temp_video_latents.shape[1], 1, 1, 1).permute(0, 2, 1, 3, 4).float()
                    opflow = opflow.reshape(video_latents.shape[0], -1)
                    opflow = opflow.to(accelerator.device) 
                ...
                model_output = transformer(
                        ...
                        encoder_hidden_mask=prompt_masks,
                        identity_prompt_embeds=identity_prompt_embeds,
                        timestep=timesteps,
                        ofs=ofs_emb,
                ...
                if len(valid_indices) != 0:
                    ...
                    opflow.to(torch.float64)
                    loss = loss + opflow * loss
                    
                    # Always check mask loss (is_train_face is always True)
                    if args.enable_mask_loss and enable_mask_loss_flag:
                        loss = (loss * dense_masks).sum() / dense_masks.sum()
                    else:
                        loss = torch.mean(loss, dim=1).mean()
                else:
                    # Fallback when no valid indices (should not happen when is_train_face is always True)
                        ...
                        if accelerator.is_main_process:
                            if args.validation_prompt is not None and args.is_validation:
                                ...
                                with torch.no_grad():
                                    # Create pipeline
                                    if args.train_type == 'i2v':
                                        pipe = ProteusIDPipeline.from_pretrained(
                                    ...
                                    validation_identity_prompts = args.validation_identity_prompt.split(args.validation_prompt_separator)
                                    validation_images = args.validation_images.split(args.validation_prompt_separator)

                                    for validation_image, validation_prompt, validation_identity_prompt in zip(validation_images, validation_prompts, validation_identity_prompts):
                                        # Always process face embeddings for validation (is_train_face is always True)
                                        B_valid_num = torch.tensor([1], dtype=torch.int32)
                                        id_image = np.array(Image.open(validation_image).convert("RGB"))
                                        id_image = resize_numpy_image_long(id_image, 1024)
                                        id_cond, id_vit_hidden, align_crop_face_image, kps_cond = process_face_embeddings(face_helper_1, face_clip_model, face_helper_2,
                                                                                                                          eva_transform_mean, eva_transform_std,
                                                                                                                          face_main_model, accelerator.device, weight_dtype,
                                                                                                                          id_image, original_id_image=id_image, is_align_face=args.is_align_face)
                                        
                                        log_valid_id_conds = []
                                        log_valid_id_conds.append(id_cond)
                                        log_valid_id_vit_hiddens = []
                                        log_valid_id_vit_hiddens.append(id_vit_hidden)
                                        log_valid_id_conds = torch.stack(log_valid_id_conds)
                                        log_valid_id_vit_hiddens = [torch.cat(tensor_group, dim=0) for tensor_group in zip(*log_valid_id_vit_hiddens)]
                                        ...
                                        if args.train_type == 'i2v':
                                            pipeline_args = {
                                                ...
                                                "identity_prompt": validation_identity_prompt,
                                        ...
                                        validation_outputs = log_validation(
                                            ...
                                            id_vit_hidden=log_valid_id_vit_hiddens if log_valid_id_vit_hiddens is not None else None,
                                            id_cond=log_valid_id_conds if log_valid_id_conds is not None else None,
