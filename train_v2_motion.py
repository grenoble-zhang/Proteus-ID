...
def main(args):
    ...
    transformer_additional_kwargs={
        ...
        'is_train_face': True,  # Always True
        'is_kps': args.is_kps,
    ...
    def compute_raft(video_tensor, human_mask, temporal_batch_size=10):
        ...
        for i in range(0, T-1, temporal_batch_size):
            ...
            predicted_flow = predicted_flow[-1]  # [B*curr_batch_size, 2, H, W]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                with torch.no_grad():
                    ...
                    batch_raft_video = batch["raft_video"] #(2,49,3,480,720)
                    ...
                    if face_clip_model is not None and face_main_model is not None:
                        ...
                        for idx, id_image in enumerate(expand_face_imgs):           # id_image: torch.Size([3, 480, 480])
                            ...
                            try:
                                ...
                                id_image = resize_numpy_image_long(id_image, 1024) # (480, 480, 3)
                                original_id_image = None # None when is_align_face is True
                                ...
                                # id_vit_hidden list(5, torch.Size([1, 577, 1024]))
                                # align_crop_face_image torch.Size([1, 3, 512, 512]) # Image with only face
                                # face_kps face keypoint coordinates (5, 2)
                    ...
                    if len(valid_indices) != 0:
                        ...
                        for idx, (video, face_img) in enumerate(zip(batch_instance_video, align_crop_face_imgs)):
                            ...
                            pil_img = Image.fromarray(tensor) # Cropped face image (512,512)
                            if args.train_type == 'i2v':
                                image = video_processor.preprocess(pil_img, height=args.height, width=args.width).to(memory_format=torch.contiguous_format).to(accelerator.device, dtype=weight_dtype) # torch [B, C, H, W] 480,720
                            ...
                            if args.train_type == 'i2v':
                                ...
                                if args.is_kps:
                                    ...
                                    image_latents = torch.cat([image_latents, face_kps_latents], dim=1) # torch.Size([1, 2, 16, 60, 90]) 
                ...
                if len(valid_indices) != 0:
                    ...
                    temp_video_latents = torch.zeros((batch_size, num_channels, num_frames, height, width))                        
                    opflow = resize_mask(opflow, temp_video_latents, process_first_frame_only=False)  # torch.Size([2, 1, 13, 60, 90]),  B C F H W
                    if patch_size_t is not None:
                        ncopy = opflow.shape[2] % patch_size_t
                        # Copy the first frame ncopy times to match patch_size_t
                        first_frame = opflow[:, :, :1, :, :]  # Get first frame [B, C, 1, H, W]
                        opflow = torch.cat([first_frame.repeat(1, 1, ncopy, 1, 1), opflow], dim=2)
                        assert opflow.shape[2] % patch_size_t == 0
                    opflow = opflow.repeat(1, temp_video_latents.shape[1], 1, 1, 1).permute(0, 2, 1, 3, 4).float()  # B C F H W -> B F C H W
