...
def generate_video(
    ...
    if upscale_model:
        del upscale_model
    if frame_interpolation_model:
        del frame_interpolation_model
    free_memory()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate a video from a text prompt using ConsisID")
    # ckpt arguments
    parser.add_argument("--model_path", type=str, default="/nfs/dataset-ofs-voyager-research/guiyuzhang/proteusid-code/ProteusID/CogVideoX-5b-I2V", help="The path of the pre-trained model to be used")
    parser.add_argument("--model_transformer_path", type=str, default="/nfs/dataset-ofs-voyager-research/shichen/project/ProteusID-v2/output_v2_moton_from_consisid_56gpu_500step/checkpoint-75", help="The path of the pre-trained model to be used")
    ...
    parser.add_argument("--identity_prompt", type=str, required=True, help="Identity prompt for video generation")
    ...
    # output arguments
    parser.add_argument("--output_path", type=str, default="./infer_output_v2_moton_from_consisid_56gpu_500step/ckpt75", help="The path where the generated video will be saved")
    ...
    '''
    CUDA_VISIBLE_DEVICES=7 python final-single.py \
  --img_file_path "/nfs/dataset-ofs-voyager-research/shichen/project/video_diffusion/RigFace/77yu/video/proteus-evaluation/final/evaluation_count/1-1-stars_man_Benedict_Cumberbatch_1.png" \
  --prompt "In an indoor office environment, characterized by horizontal blinds that allow soft natural light to filter through, a man with short dark hair, dressed in a formal black suit with a white shirt and tie, captures the viewer's attention. His expression appears neutral or contemplative as he initially looks upwards and slightly to the side before turning his head to face forward. With no significant movement other than this slight head turn, the camera remains static, focusing closely on his upper body and face, enhancing the professional and calm atmosphere of the scene." \
  --identity_prompt "A man with short dark hair, wearing a formal black suit with a white shirt"
    '''
    
    
    '''
    "cap": "In an indoor office environment, characterized by horizontal blinds that allow soft natural light to filter through, a man with short dark hair, dressed in a formal black suit with a white shirt and tie, captures the viewer's attention. His expression appears neutral or contemplative as he initially looks upwards and slightly to the side before turning his head to face forward. With no significant movement other than this slight head turn, the camera remains static, focusing closely on his upper body and face, enhancing the professional and calm atmosphere of the scene.",
    "identity": "A man with short dark hair, wearing a formal black suit with a white shirt",
    "image_path": "/nfs/dataset-ofs-voyager-research/shichen/project/video_diffusion/RigFace/77yu/video/proteus-evaluation/final/evaluation_count/1-1-stars_man_Benedict_Cumberbatch_1.png"
    '''
