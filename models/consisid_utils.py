...
def process_face_embeddings(
    ...
    if len(face_info) > 0:
        ...
        face_kps = face_info["kps"] # 当前人脸的关键点坐标 (5, 2)
    else:
    ...
    face_helper_1.align_warp_face() # 将人脸align到标准位置
    if len(face_helper_1.cropped_faces) == 0:
        raise RuntimeError("facexlib align face fail")
    align_face = face_helper_1.cropped_faces[0]  # (512, 512, 3)  # RGB align后的人脸
    ...
    if is_align_face:
        ...
        return_face_features_image = torch.where(bg, white_image, to_gray(input))  # torch.Size([1, 3, 512, 512]) #只有人脸的图片
        return_face_features_image_2 = torch.where(bg, white_image, input)  # torch.Size([1, 3, 512, 512]) #只有人脸的图片
    else:
