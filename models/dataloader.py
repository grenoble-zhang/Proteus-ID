...
def check_dictionaries(dict1, dict2, dict1_name="dict1", dict2_name="dict2"):
    """
    Check if keys and values of two dictionaries are completely consistent.
    
    Args:
        dict1, dict2: Two dictionaries to compare.
        dict1_name, dict2_name: Names of dictionaries for output.
    
    Returns:
        bool: Whether the two dictionaries are completely consistent.
    """
    keys1 = set(dict1.keys())
    keys2 = set(dict2.keys())
    
    if keys1 != keys2:
        print(f"Keys inconsistent:")
        print(f"Keys only in {dict1_name}: {keys1 - keys2}")
        print(f"Keys only in {dict2_name}: {keys2 - keys1}")
        return False
    ...
    for key in keys1:
        ...
        if len(values1) != len(values2):
            print(f"Key '{key}' value list length inconsistent: {dict1_name}={len(values1)}, {dict2_name}={len(values2)}")
            all_equal = False
            continue
        
        for i, (val1, val2) in enumerate(zip(values1, values2)):
            if not np.array_equal(val1, val2):
                print(f"Key '{key}' value {i} inconsistent")
                if val1.shape != val2.shape:
                    print(f"  Shape inconsistent: {dict1_name}={val1.shape}, {dict2_name}={val2.shape}")
                else:
                    num_diff = np.sum(val1 != val2)
                    percent_diff = 100.0 * num_diff / val1.size
                    print(f"  {num_diff} elements inconsistent ({percent_diff:.2f}%)")
                    
                    if val1.size < 100:
                        print(f"  {dict1_name} value: {val1}")
                        print(f"  {dict2_name} value: {val2}")
                        print(f"  Difference positions: {np.where(val1 != val2)}")
                
                all_equal = False
    
    if all_equal:
        print(f"{dict1_name} and {dict2_name} are completely consistent")
    else:
        print(f"{dict1_name} and {dict2_name} have differences")
    
    return all_equal

def check_dictionaries_equality(dict1, dict2, dict1_name="dict1", dict2_name="dict2"):
    """
    Check if keys and values of two dictionaries are completely consistent, and return index positions of inconsistent values.
    ...
    Returns:
        bool: Whether the two dictionaries are completely consistent.
        dict: Dictionary containing index position information of inconsistent values.
    """
    differences = {}
            ...
            if not np.array_equal(val1, val2):
                ...
                if val1.shape != val2.shape:
                    ...
                    if val1.size < 100:
                        print(f"  {dict1_name} value: {val1}")
                        print(f"  {dict2_name} value: {val2}")
                        print(f"  Difference positions: {diff_indices}")
    ...
    def get_batch(self, idx):
        ...
        with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
            ...
            with open(bboxs_data_path, 'r') as f:
                ...
                human_masks_tensor = self._short_resize_and_crop(human_temp_dense_masks_tensor.unsqueeze(-1), self.width, self.height).squeeze(-1)
            ...
            raft_pixel_values = frames.permute(0, 3, 1, 2).contiguous()
            del video_reader
