...
from diffusers.models import AutoencoderKLCogVideoX, ProteusIDTransformer3DModel
from diffusers.models.embeddings import get_3d_rotary_pos_embed
from diffusers.pipelines.proteusid.pipeline_output import ProteusIDPipelineOutput
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import ProteusIDPipeline
        >>> from diffusers.pipelines.proteusid.proteusid_utils import prepare_face_models, process_face_embeddings_infer
        >>> from diffusers.utils import export_to_video
        >>> from huggingface_hub import snapshot_download

        >>> snapshot_download(repo_id="BestWishYsh/ProteusID-preview", local_dir="BestWishYsh/ProteusID-preview")

        >>> face_helper_1, face_helper_2, face_clip_model, face_main_model, eva_transform_mean, eva_transform_std = (
        ...     prepare_face_models("BestWishYsh/ProteusID-preview", device="cuda", dtype=torch.bfloat16)
        ... )
        >>> pipe = ProteusIDPipeline.from_pretrained("BestWishYsh/ProteusID-preview", torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
        ...
        >>> image = "https://github.com/PKU-YuanGroup/ProteusID/blob/main/asserts/example_images/1.png?raw=true"
...
class ProteusIDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __call__(
        ...
        kps_cond: Optional[torch.Tensor] = None,
    ) -> Union[ProteusIDPipelineOutput, Tuple]:
        """
        ...
        Returns:
            [`~pipelines.proteusid.pipeline_output.ProteusIDPipelineOutput`] or `tuple`:
            [`~pipelines.proteusid.pipeline_output.ProteusIDPipelineOutput`] if `return_dict` is True, otherwise a
        ...
        return ProteusIDPipelineOutput(frames=video)
