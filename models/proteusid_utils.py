...
def process_face_embeddings(
    ...
    # get antelopev2 face_encoder embedding
    ...
    if len(face_info) > 0:
        ...
        face_kps = face_info["kps"]  # Current face keypoint coordinates (5, 2)
    else:
    ...
    face_helper_1.align_warp_face()  # Align face to standard position
    if len(face_helper_1.cropped_faces) == 0:
        raise RuntimeError("facexlib align face fail")
    align_face = face_helper_1.cropped_faces[0]  # (512, 512, 3) RGB aligned face
    ...
    if is_align_face:
        ...
        return_face_features_image = torch.where(bg, white_image, to_gray(input))  # torch.Size([1, 3, 512, 512]) Image with face only
        return_face_features_image_2 = torch.where(bg, white_image, input)  # torch.Size([1, 3, 512, 512]) Image with face only
    else:
...
def encode_prompt(
    ...
    prompt = [prompt] if isinstance(prompt, str) else prompt
    prompt_embeds, text_input_mask = _get_t5_prompt_embeds(
        tokenizer,
        text_encoder,
        prompt=prompt,
...
def compute_prompt_embeddings(
    tokenizer, text_encoder, prompt, max_sequence_length, device, dtype, requires_grad: bool = False, mask_return: bool = False
):
    """
    ...
    if requires_grad:
        prompt_embeds, text_input_mask = encode_prompt(
            tokenizer,
            text_encoder,
            prompt,
