# Adapted from Identity-Preserving Text-to-Video Generation by Frequency Decomposition (https://arxiv.org/abs/2411.17440)
...
class ProteusIDPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    Pipeline for image-to-video generation using ProteusID.
    ...
    Args:
        ...
        text_encoder ([`T5EncoderModel`]):
            Frozen text-encoder. ProteusID uses
            [T5](https://huggingface.co/docs/transformers/model_doc/t5#transformers.T5EncoderModel); specifically the
            [t5-v1_1-xxl](https://huggingface.co/PixArt-alpha/PixArt-alpha/tree/main/t5-v1_1-xxl) variant.
        tokenizer (`T5Tokenizer`):
            Tokenizer of class
            [T5Tokenizer](https://huggingface.co/docs/transformers/model_doc/t5#transformers.T5Tokenizer).
        transformer ([`ProteusIDTransformer3DModel`]):
            A text conditioned `ProteusIDTransformer3DModel` to denoise the encoded video latents.
        scheduler ([`SchedulerMixin`]):
    ...
    def _get_t5_prompt_embeds(
        ...
        text_input_mask = text_inputs.attention_mask
        ...
        text_input_mask = text_input_mask.repeat(1, num_videos_per_prompt)
        text_input_mask = text_input_mask.view(batch_size * num_videos_per_prompt, seq_len)

        return prompt_embeds, text_input_mask

    # Copied from diffusers.pipelines.cogvideo.pipeline_cogvideox.CogVideoXPipeline.encode_prompt
    def encode_identity_prompt(
        self,
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_embeds, negative_prompt_embeds_mask = self._get_t5_prompt_embeds(
    ...
    def check_inputs(
        ...
        # identity prompt
        if identity_prompt is None and identity_prompt_embeds is None:
            raise ValueError(
                "Provide either `identity_prompt` or `identity_prompt_embeds`. Cannot leave both `identity_prompt` and `identity_prompt_embeds` undefined."
            )
    ...
    def __call__(
        ...
        identity_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        Args:
            ...
            num_frames (`int`, defaults to `49`):
                ...
                contain 1 extra frame because ProteusID is conditioned with (num_seconds * fps + 1) frames where
        ...
        prompt_embeds, prompt_embeds_mask, negative_prompt_embeds, negative_prompt_embeds_mask = self.encode_prompt(
            prompt=prompt,
            negative_prompt=negative_prompt,
        ...
        identity_prompt_embeds = self.encode_identity_prompt(
            prompt=identity_prompt,
            num_videos_per_prompt=num_videos_per_prompt,
            max_sequence_length=max_identity_length,
            device=device,
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    encoder_hidden_mask=prompt_embeds_mask,
                    identity_prompt_embeds = identity_prompt_embeds,
                    timestep=timestep,
                    ofs=ofs_emb,
